...
- PNG 保存、编号、覆盖确认和真实分页预览已接入并通过离屏回归；下一轮需重新建立独立合同。
- 导出放大预览打印图标只绑定功能，不调整头部布局；PDF / PNG 按当前真实分页文档打印，Markdown 打印当前文本预览。高 DPI 打印允许限制中间栅格缓存尺寸，但纸面铺放比例必须保持完整。
- 详情弹窗的重要性 / 重复规则选项菜单统一读取 `StyleManager.theme_accent_color()` 和 `theme_overlay_rgba(...)`，不得保留固定青色。
- `ActionContextMenu` 模式 / 视图二级菜单本轮只复核，不修改锁定和关闭时序；只有稳定复现失败后才建立独立修复工单。
