...
## 2026-07-06 坐标看板信息弹窗主题化

- 开工前将坐标看板真实设置接线、文字绘制层级和刷新补齐提交为检查点 `750f194`，提交说明为 `feat: wire axis board display settings`。
- `_AxisSchedulePreview` 外层背景改为与主界面相同的数据源和绘制规则：读取 `AppConfig.COLOR_GRADIENT_START / COLOR_GRADIENT_END` 绘制纵向渐变；上下沿颜色相同时自然表现为纯色，不单独增加纯色分支，图片皮肤暂不处理。
- 弹窗内部新增距外层约 `6px` 的显示框，参数与坐标看板真实显示框一致：`2px` 白色边缘、`rgba(255,255,255,190)` 半透明白色遮罩和 `6px` 圆角；内容边距调整为在内框中继续保留约 `9-11px` 的文字留白。
- 日程标题颜色读取主题背景上沿色；时间、清单、重要性、完成状态、持久弹窗提示和关闭键颜色读取主题背景下沿色。每次设置新投影时重新应用当前 `AppConfig` 颜色，避免弹窗实例长期持有旧主题文字颜色。
- hover 弹窗的鼠标穿透、屏幕边界回退和移开隐藏保持不变；持久弹窗的关闭键和既有详情打开回退路径保持不变。
- 工作区内置 Python 3.12 无缓存 `py_compile` 通过，`git diff --check` 无 whitespace error，结构检查确认内框边距、遮罩、渐变和文字颜色接线均存在。
- 本轮项目 `.venv` 的沙箱外执行请求因 Codex 工具额度限制被拒绝，不是 `.venv` 故障；因此未声明 PyQt offscreen 截图验收通过，需在 Windows 实机确认渐变边缘、6px 间距、纯色主题和长标题换行效果。

## 2026-07-06 安全加密入口与月弹窗主题边框审查

- 接收并审查用户后续新增改动：更多菜单将“待办显示”统一为“待办看板”，新增“安全加密”入口和 `EncryptionWindow` 占位窗口；月日期 hover 预览改为带透明度的主题渐变、白色文字和 `2px` 白边，月日期持久 panel 增加内层白色轮廓。
- 审查发现 `EncryptionWindow` 的关闭按钮只创建但未加入标题布局，实际会停留在错误位置；已将按钮加入标题行右侧并保持原关闭行为。同步删除窗口中未使用的置顶状态、导入和按钮样式占位。
- 安全加密窗口首次打开时增加当前屏幕可用区域约束：优先显示在来源窗口右侧，右侧空间不足则回退左侧，再统一夹紧横纵坐标；显示时补充 `raise_()` 和 `activateWindow()`，避免窗口创建后落到来源窗口后方。
- 月日期持久 panel 经后续实机调整，当前最终状态为距边缘 `4px` 的 `2px` 白色内框且不填充整块遮罩；日程项自身白色背景由 `12%` 调为 `15%` 不透明度，以局部层次替代整框遮罩。其他布局、详情路由和状态切换逻辑未改动。
- 新增 `assets/icons/security_lock.svg` 已通过 Qt SVG 加载验证；安全加密入口仍为明确的“功能待实现”UI 壳，不包含真实加密、密钥或文件写入逻辑。
- 项目 `.venv` 下目标文件 `py_compile` 通过。PyQt offscreen 验证通过：加密窗口关闭键位于标题右侧且可见，更多菜单可显示 / 隐藏窗口，窗口在右侧空间不足时保持在屏幕可用区域内；安全图标可加载，月 hover 预览和月持久 panel 均可构造并完成离屏绘制。临时验证脚本已清理。

## 2026-07-06 本地数据库加密与多账号规划记录

- 按用户要求仅同步长期规划，不启动源码实现。`Final_Formulation.md` 在云功能之前新增“本地数据库加密与本地多账号”专项，并明确其当前为低优先级特殊需求。
- 规划记录 SQLCipher / `sqlcipher3` / Peewee 延迟初始化方向、现有 `db_manager` 导入即连接的前置问题、`sqlcipher_export()` 明密文迁移与回滚、更换密码、KDF 职责、启动 / 挂起锁定边界和密钥内存限制。
- 本地多账号暂定为“一账号一数据库”，在整库加密之后实施，为后续云账号保留稳定 profile ID；信封解锁动画与数据库安全逻辑拆分，先完成隔离技术样本再进入真实数据库改造。
