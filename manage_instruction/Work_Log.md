...
## 2026-06-17 日程显示模式切换规划补充

- 用户问题：
  - 评估是否可以把后续“课表形态 / 日程卡片形态”切换放入右上角更多菜单。
  - 日界面和周界面未来都可支持两种显示形态。
  - 月界面格子过小，不适合直接展示细节，但单击日期持久弹窗后续可能做成类似日界面下方内容区，也可受模式影响。
- 本次源码审查：
  - `src/ui/components.py`：
    - `SharedMoreMenu` 已是主 / 周 / 月窗口共用的更多菜单。
    - 当前已存在“使用帮助”二级菜单结构，可作为后续“模式切换”二级菜单的实现参考。
    - `SharedMoreMenu` 当前直接持有 `parent_window`，适合做轻量入口和信号出口，但不适合直接保存业务显示模式或重绘日程。
  - `src/ui/main_window.py`：
    - 当前已有集中 `switch_view(...)` 和 `_sync_view_selector_state(...)`，适合后续承接全局显示模式状态。
  - `src/ui/dashboard.py`：
    - 日界面当前使用 `ScheduleCard` 渲染日程卡片。
    - `refresh_data()` 是后续按显示模式切换卡片 / 课表渲染的主要边界。
  - `src/ui/week_window.py`：
    - 周界面当前使用 `WeekScheduleCard` 渲染日程卡片。
    - `load_week_schedules_from_db()` 是后续按显示模式切换卡片 / 课表渲染的主要边界。
  - `src/ui/popups/month_day_panel.py`：
    - 月界面单击日期持久弹窗当前只做简版摘要列表。
    - 后续如将其做成类似日界面下方内容区，可支持卡片 / 课表两种展示并进入修改 / 删除流程。
- 评估结论：
  - 将“卡片模式 / 课表模式”作为全局日程显示偏好放入更多菜单是合理的。
  - 更多菜单主项建议命名为“模式切换”，右侧二级菜单包含“卡片模式”和“课表模式”。
  - 二级菜单应高亮当前模式，两项互斥。
  - 切换后应影响所有相关日程界面，并在后续切换日 / 周 / 月时保持一致。
  - `SharedMoreMenu` 不应直接持有模式状态；后续应由 `MainWindow`、轻量状态对象或 coordinator 统一维护，再同步给日界面、周界面和月界面相关弹窗。
- 文档改动：
  - `manage_instruction/Final_Formulation.md`：
    - 在“日界面 / 周界面课程表视图”条目下补充“日程显示模式”规划。
    - 明确卡片模式、课表模式、更多菜单入口、互斥高亮、全局偏好和月界面持久弹窗可能受模式影响。
- 范围说明：
  - 本次不修改 `src/`。
  - 本次不新增菜单项或图标。
  - 本次不实现课表模式。
  - 后续需等待用户提供模式切换图标后，再单独执行更多菜单入口和信号壳。

---

## 2026-06-17 更多菜单模式切换入口壳

- 背景：
  - 用户已提供 `model_switch.svg`、`schedule_card.svg`、`timetable.svg`。
  - 前一轮已确认“卡片模式 / 课表模式”适合做成全局日程显示偏好入口。
  - 本轮只做更多菜单入口和二级菜单壳，不接真实日 / 周 / 月渲染。
- 本次代码改动：
  - `src/ui/components.py`：
    - `SharedMoreMenu` 新增“模式切换”主项。
    - 新增 `self.mode_menu` 二级菜单，包含“卡片模式”和“课表模式”。
    - 使用 `model_switch.svg`、`schedule_card.svg`、`timetable.svg`。
    - 新增 `_schedule_display_mode` 占位状态，默认 `card`。
    - 当前模式项使用 `rgba(12, 192, 223, 0.16)` 高亮，两项互斥。
    - 点击模式项只更新菜单内部占位状态和高亮，打印“功能待接入”，不改变任何日 / 周 / 月渲染。
    - “模式切换”和“使用帮助”两个二级菜单互斥显示，打开一个会关闭另一个。
    - 模式二级菜单展开时，主菜单“模式切换”行保持 hover 高亮；鼠标离开主项和二级菜单区域后关闭并清除高亮。
- 本次文档改动：
  - `manage_instruction/Final_Formulation.md`：
    - 在“日界面 / 周界面课程表视图”条目中记录当前进度。
    - 明确更多菜单模式切换壳已完成，真实渲染切换仍待后续单独实现。
- 范围说明：
  - 本次不修改 `MainWindow`、`DashboardView`、`WeekWindow`、`MonthWindow` 的真实日程渲染。
  - 本次不新增数据库字段。
  - 本次不接入 controller 或 global signal。
  - 本次不实现课表模式。
- 验证记录：
  - 静态定位“模式切换 / 卡片模式 / 课表模式 / mode_menu / `_schedule_display_mode`”：通过。
  - `py_compile` 覆盖 `src/ui/components.py`、`main.py`：通过。
  - offscreen 验证 `SharedMoreMenu.mode_menu` 包含“卡片模式 / 课表模式”：通过。
  - offscreen 验证默认模式为 `card` 且卡片模式高亮：通过。
  - offscreen 验证切换到 `timetable` 后课表模式高亮、卡片模式取消高亮：通过。
  - offscreen 验证三个图标均可作为 `QIcon` 加载：通过。
- 风险和后续：
  - 当前 `_schedule_display_mode` 只是 UI 壳占位，未持久化、未广播、未驱动真实界面。
  - 后续实装课表模式时，应另开工单接入统一状态来源，再改日 / 周 / 月弹窗渲染。
- 后续交互修正：
  - 用户反馈“模式切换”和“使用帮助”的二级弹窗在鼠标移向子菜单时容易关闭，导致无法稳定点击二级选项。
  - `src/ui/components.py`：
    - 为“模式切换”新增 `_mode_menu_locked` 状态。
    - 为“使用帮助”新增 `_help_menu_locked` 状态。
    - 悬停主项时仍临时展开二级菜单，鼠标离开主项和二级菜单区域后自动关闭。
    - 单击主项时锁定二级菜单，鼠标移走不自动关闭，便于点击二级选项。
    - 再次单击已锁定主项时取消锁定，后续鼠标离开后恢复自动关闭。
    - 打开“模式切换”会关闭并解锁“使用帮助”；打开“使用帮助”会关闭并解锁“模式切换”。
    - 主菜单关闭时重置两个锁定状态、停止 hover timer，并清除主项 hover 高亮。
  - 该修正只改变更多菜单二级菜单生命周期，不接入真实卡片 / 课表渲染，不修改帮助文档或帮助助手功能。
  - 验证记录：
    - `py_compile` 覆盖 `src/ui/components.py`、`main.py`：通过。
    - offscreen 验证“模式切换”和“使用帮助”主项单击锁定、再次单击解锁、互斥关闭和主菜单关闭重置锁定状态：通过。
