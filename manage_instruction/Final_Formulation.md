...
1. 修改四象限展示形式。
   ...
   将四象限从完整独立界面降级为轻量看板 / 弹窗 / 面板，先明确数据来源、入口位置和交互边界。

2. 待办看板文字截断修复。

   修复便签标题和括号内容显示不完整的问题，优先保证 2 行内可读，超长再省略；不得混入拖拽、排序、分类写库逻辑。
...
4. 实现四象限显示。
   ...
   基于已有日程 / 待办数据做四象限展示，先支持查看、修改、删除，不提供新增入口。实现前必须确认“重要性”字段来源；若当前数据模型不足，应单独规划字段或降级规则。
...
9. 天气服务失败兜底。
   ...
   增强天气接口异常、空响应、非 JSON 响应时的兜底展示和日志记录。
