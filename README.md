...
最终阶段（云同步及其他功能完整实装后执行）：

- UI 审美评估：当前界面已经形成渐变主题、统一图标语言和日/周/月视图结构，整体辨识度较好；后续重点不是推翻现有风格，而是提升层级、留白、可读性和弹窗一致性。
- 设计方向：保持“克制、清透、低干扰的桌面日程玻璃面板”风格，主题色主要用于品牌、选中和状态反馈，避免大面积同色铺满。
- 第一阶段：统一字体、字号、间距、圆角、透明度、阴影和控件尺寸等设计变量，并提高过小辅助文字在高 DPI 下的可读性。
- 第二阶段：减少普通区域的白色描边和多层半透明叠加，优先通过背景层级、轻阴影和留白划分区域。
- 第三阶段：统一主界面及各弹窗的标题栏高度、图标对齐、按钮热区和内容间距，优先整理信息较拥挤的纪念日期弹窗。
- 第四阶段：补充克制的点击、弹出和拖拽边界反馈，并兼顾减少动态效果、减少透明度等可访问性设置。
- 实施约束：在功能开发期间只修复影响操作和可读性的 UI 问题，不进行大范围视觉重构；所有功能稳定后再集中修改和回归检查。
